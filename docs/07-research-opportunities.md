...
### 第一梯队：范式级机会（新发现，比原有 6 个影响力更大）

| 编号 | 方向 | 空白度 | 影响力 | 时效性 |
|------|------|--------|--------|--------|
| **A** | 自回归视频生成加速 | ✅ 极高 (2026.02 刚开始) | 极高 (范式迁移) | 紧迫 |
...
视频生成正在从**双向 DiT → 自回归**范式迁移：

```
双向 DiT (当前主流)              自回归 (正在崛起)
┌─────────────────────┐        ┌─────────────────────┐
│ 一次性生成所有帧      │        │ 逐 chunk 自回归生成   │
│ Wan2.1, HunyuanVideo │   →    │ MAGI-1, SkyReels-V2  │
│ 加速研究：非常成熟     │        │ Self-Forcing, CausalWan│
│ (我们调研的 6 个方向)  │        │ 加速研究：刚刚开始     │
└─────────────────────┘        └─────────────────────┘
```

**我们原有 6 个机会全部面向双向 DiT。** 如果范式迁移完成，这些可能变成 legacy。
...
- AR 模型正在快速迭代（MAGI-1、SkyReels-V2、LongCat、Self-Forcing++ 都在 2025 下半年发布）
- LLM 推理优化社区（vLLM/SGLang）正在进入这个领域 — 他们有工程优势但缺乏视频领域知识
...
```
路径 1 (追求最大影响): A (AR视频加速) — 范式迁移窗口期，竞争刚开始
路径 2 (追求理论深度): B (测试时计算×加速) — 改变问题定义，完全空白
路径 3 (追求低风险):   1 (DuCa理论) + X1 (Reward缓存) — 空白确认，工程量低
路径 4 (追求工程落地): A + 4 (AR加速 + 自适应框架) — 面向下一代视频服务
```
